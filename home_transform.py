...
def home_transform(input_data, output_data):
    ...
    cols = [x for x in df_home.columns]
    
    #remove Y column and columns whose data has been processed
    drop_list = ['Office - Computer Density','Office - Weekly Operating Hours','Office - Worker Density',
    ...
    df_home_cleaned = df_home[cols]

    with open(output_data, 'w') as output_file:
        output_file = df_home_cleaned.to_csv(index=False)


home_transform(input_data,output_data)
